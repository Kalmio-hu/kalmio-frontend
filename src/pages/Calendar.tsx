/**
 * Calendar — materialized planned_meal calendar view.
 *
 * Route: /app/calendar
 *
 * Default window: current week + next week (14 days).
 * Mobile:   vertical stack of day cards, each showing meals for that day.
 * Desktop:  weekly grid — days as columns, meal-slot rows, member sub-cells.
 * Navigation: prev/next week buttons, Today button, date picker for jumping.
 *
 * Data source: GET /api/planned-meals?from=&to= (KALMIO-249 backend follow-up).
 * Until that endpoint ships, the service returns [] and the empty-state is shown.
 *
 * KALMIO-237 / C15
 */
...
export function Calendar() {
  ...
  const { mutate: patchStatus, isPending: isPatching } = useMutation({
    ...
    onSuccess: (updated) => {
      ...
      // Update the selected meal in the panel so the UI reflects immediately
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 max-w-5xl mx-auto w-full pb-24 md:pb-6">
        {/* Navigation bar */}
